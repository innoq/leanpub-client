...
class CouponJsonSpec extends WordSpec with TypeCheckedTripleEquals {
  ...
  "The Coupon Class" when {
    "reading JSON" which {
      "is valid and contains all Coupon Data except from max_uses" should {
        "return a Coupon Object with all fields and None for maxUses" in {
          val json = """{
            "coupon_code": "test",
            "created_at": "2012-01-01T12:00:00Z",
            "package_discounts": [
          ...
          assert(coupon === Coupon("test", createdAt, List(PackageDiscount("book", 2.0)),
            endDate, None, "This is not a real coupon", 12,
            startDate, suspended = false, updatedAt,
            "yourbook"))
        }
      }
